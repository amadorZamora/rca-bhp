...
const ADMIN_EMAIL  = "amador.zamora@gmail.com";
const ADMIN_PW     = "bhp2024";
const ALL_PHASES   = ["board","votar","porques","acciones","resumen"];
const PHASE_LABELS: Record<string,string> = { board:"📝 Board", votar:"🗳️ Votar", porques:"🔍 5 Porqués", acciones:"✅ Acciones", resumen:"📊 Resumen" };
...
export default function App() {
  // ── Auth state
  const [screen,    setScreen]    = useState<"login"|"admin"|"user">("login");
  ...
  // ── Admin panel state
  const [adminTab,     setAdminTab]     = useState<"projects"|"new"|"edit">("projects");
  ...
  const handleLogin = async () => {
    ...
    if (loginEmail.trim().toLowerCase() === ADMIN_EMAIL.toLowerCase()) {
      if (loginPw !== ADMIN_PW) return setLoginErr("Contraseña incorrecta.");
      setIsAdmin(true); setUserName("Admin"); setUserEmail(ADMIN_EMAIL);
      setScreen("admin");
    } else {
      if (!loginName.trim()) return setLoginErr("Ingresa tu nombre.");
      setUserName(loginName.trim()); setUserEmail(loginEmail.trim());
      setScreen("user");
    }
  };
  ...
  const createProject = async () => {
    ...
    await setDoc(doc(db,"projects",id), {
      ...
      phases, createdAt: serverTimestamp(), status:"active"
    });
  ...
  const deleteProject = async (id: string) => {
    if (!confirm("¿Eliminar este proyecto? Se borrarán todos sus datos.")) return;
  ...
  const clearProjectData = async (pid: string) => {
    ...
    const aSnap = await getDocs(collection(db,"projects",pid,"actions"));
    for (const d of aSnap.docs) await deleteDoc(d.ref);
    const pSnap = await getDocs(collection(db,"projects",pid,"porques"));
  ...
  const addNote = async (colId: string) => {
    if (!selectedProject) return;
    const id = uid();
    await setDoc(doc(db,"projects",selectedProject.id,"notes",colId,"items",id), {
  ...
  const addAction = async () => {
    if (!selectedProject) return;
    const id = uid();
    await setDoc(doc(db,"projects",selectedProject.id,"actions",id), {
  ...
  const activePhases = ALL_PHASES.filter(p => phases[p]);
  ...
  if (screen==="login") return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1565C0 0%,#0D47A1 100%)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:20, padding:40, maxWidth:420, width:"90%", boxShadow:"0 8px 40px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div>
            <label style={{ fontSize:12, color:"#555", fontWeight:600 }}>Nombre</label>
            <input value={loginName} onChange={e=>setLoginName(e.target.value)} placeholder="Tu nombre completo"
          ...
          {loginEmail.toLowerCase()===ADMIN_EMAIL.toLowerCase() && (
            <div>
              <label style={{ fontSize:12, color:"#555", fontWeight:600 }}>Contraseña (Admin)</label>
          ...
          <button onClick={handleLogin}
            style={{ background:"#1565C0", color:"#fff", border:"none", borderRadius:8, padding:"12px", fontSize:15, cursor:"pointer", fontWeight:700, marginTop:4 }}>
            {loginEmail.toLowerCase()===ADMIN_EMAIL.toLowerCase() ? "Entrar como Administrador 🔑" : "Entrar →"}
          </button>
        </div>
      </div>
    </div>
  ...
  if (screen==="admin") return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8", fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:"#1a237e", color:"#fff", padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <span style={{ fontSize:18, fontWeight:700 }}>🪨 RCA Platform · Panel Administrador</span>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <span style={{ fontSize:13, opacity:0.8 }}>👤 {ADMIN_EMAIL}</span>
          <button onClick={()=>setScreen("login")} style={{ background:"rgba(255,255,255,0.15)", color:"#fff", border:"none", borderRadius:20, padding:"6px 16px", fontSize:13, cursor:"pointer" }}>
            Cerrar sesión
          </button>
        </div>
      </div>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #e0e0e0", padding:"0 24px", display:"flex", gap:4 }}>
        {([["projects","📋 Proyectos"],["new","➕ Nuevo Proyecto"]] as [string,string][]).map(([t,label])=>(
          <button key={t} onClick={()=>{ setAdminTab(t as any); setEditingProj(null); }}
        ...
        {editingProj && (
          <button style={{ background:"none", border:"none", borderBottom:"3px solid #1565C0", color:"#1565C0", padding:"14px 16px", fontSize:14, cursor:"pointer", fontWeight:700 }}>
      ...
      <div style={{ padding:"24px" }}>
              ...
              <div key={proj.id} style={{ background:"#fff", borderRadius:12, padding:20, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>{ setEditingProj({...proj}); setAdminTab("edit" as any); }}
                      style={{ background:"#E3F2FD", color:"#1565C0", border:"none", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:13 }}>✏️ Editar</button>
                    <button onClick={()=>clearProjectData(proj.id)}
                      style={{ background:"#FFF8E1", color:"#E65100", border:"none", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:13 }}>🗑️ Limpiar datos</button>
                    <button onClick={()=>exportCSV(proj)}
                      style={{ background:"#E8F5E9", color:"#2E7D32", border:"none", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:13 }}>📥 Exportar CSV</button>
                    <button onClick={()=>deleteProject(proj.id)}
                      style={{ background:"#FFEBEE", color:"#c62828", border:"none", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:13 }}>🗑️ Eliminar</button>
                  </div>
                </div>

                {/* Phase toggles */}
                <div>
                  <div style={{ fontSize:12, color:"#555", fontWeight:600, marginBottom:8 }}>FASES ACTIVAS:</div>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {ALL_PHASES.map(phase=>{
                      const active = proj.phases?.[phase] || false;
                      return (
                        <button key={phase} onClick={()=>togglePhase(proj.id,phase,active)}
                          style={{ background:active?"#1565C0":"#eee", color:active?"#fff":"#888", border:"none", borderRadius:20, padding:"6px 16px", fontSize:13, cursor:"pointer", fontWeight:active?700:400, transition:"all 0.2s" }}>
                ...
                <div style={{ marginTop:12, fontSize:12, color:"#888" }}>
                  👥 Participantes registrados: <strong style={{ color:"#333" }}>{proj.participantCount||0}</strong>
                </div>
              </div>
            ))}
          </div>
        ...
        {adminTab==="new" && (
          <div style={{ background:"#fff", borderRadius:12, padding:28, maxWidth:500, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:"#555" }}>Nombre del proyecto *</label>
                <input value={newProjName} onChange={e=>setNewProjName(e.target.value)} placeholder="Ej: Proyecto Mina Norte Q1 2025"
                  style={{ width:"100%", padding:"10px 12px", borderRadius:8, border:"1.5px solid #e0e0e0", fontSize:14, boxSizing:"border-box", marginTop:4 }} />
              </div>
              <div>
              ...
              <div style={{ background:"#E3F2FD", borderRadius:8, padding:12, fontSize:13, color:"#0D47A1" }}>
                💡 El proyecto se creará con solo el <strong>Board</strong> activo. Activa el resto de fases desde el panel de proyectos.
              </div>
        ...
        {/* Edit project */}
        {adminTab===("edit" as any) && editingProj && (
          <div style={{ background:"#fff", borderRadius:12, padding:28, maxWidth:500, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={updateProject}
                  style={{ flex:1, background:"#1565C0", color:"#fff", border:"none", borderRadius:8, padding:"12px", fontSize:14, cursor:"pointer", fontWeight:700 }}>
                  Guardar cambios
                </button>
                <button onClick={()=>{ setEditingProj(null); setAdminTab("projects"); }}
                  style={{ background:"#eee", color:"#555", border:"none", borderRadius:8, padding:"12px 20px", fontSize:14, cursor:"pointer" }}>
                  Cancelar
                </button>
              </div>
            </div>
  ...
  // ════════════════════════════════════════════════════════════════
  // ── SCREEN: USER — Project selection
  // ════════════════════════════════════════════════════════════════
  if (screen==="user" && !selectedProject) return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8", fontFamily:"'Segoe UI',sans-serif" }}>
      <div style={{ background:"#1565C0", color:"#fff", padding:"14px 24px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button onClick={()=>setScreen("login")} style={{ background:"rgba(255,255,255,0.15)", color:"#fff", border:"none", borderRadius:20, padding:"6px 14px", fontSize:13, cursor:"pointer" }}>
            Salir
          </button>
        </div>
      </div>
      <div style={{ padding:32, maxWidth:700, margin:"0 auto" }}>
        ...
        {projects.filter(p=>p.status!=="deleted").length===0 && (
          <div style={{ background:"#fff", borderRadius:12, padding:32, textAlign:"center", color:"#aaa" }}>
            No hay proyectos disponibles aún. El administrador debe crear uno.
          </div>
        )}
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {projects.filter(p=>p.status!=="deleted").map(proj=>(
                ...
                <div>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {ALL_PHASES.filter(p=>proj.phases?.[p]).map(p=>(
                      <span key={p} style={{ background:"#E3F2FD", color:"#1565C0", borderRadius:12, padding:"2px 10px", fontSize:11, fontWeight:600 }}>
                        {PHASE_LABELS[p]}
                      </span>
                    ))}
                  </div>
  ...
  // ════════════════════════════════════════════════════════════════
  // ── SCREEN: USER — Board (project selected)
  // ════════════════════════════════════════════════════════════════
  return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8", fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:"#1565C0", color:"#fff", padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <button onClick={()=>setSelectedProject(null)} style={{ background:"rgba(255,255,255,0.15)", color:"#fff", border:"none", borderRadius:20, padding:"4px 12px", fontSize:12, cursor:"pointer" }}>
            ← Proyectos
          </button>
          <div>
            <span style={{ fontSize:16, fontWeight:700 }}>🪨 {selectedProject?.name}</span>
            {saving && <span style={{ marginLeft:8, fontSize:11, opacity:0.7 }}>💾 guardando...</span>}
          </div>
        </div>
        <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
          ...
          {activePhases.map(t=>(
            <button key={t} onClick={()=>setTab(t)}
              style={{ background:tab===t?"#fff":"rgba(255,255,255,0.15)", color:tab===t?"#1565C0":"#fff", border:"none", borderRadius:20, padding:"5px 14px", fontSize:12, cursor:"pointer", fontWeight:tab===t?700:400 }}>
              {PHASE_LABELS[t]}
            </button>
          ))}
        </div>
      ...
      {tab==="votar" && <div style={{ background:"#FFF8E1", borderLeft:"4px solid #F9A825", margin:"16px 24px 0", borderRadius:8, padding:"10px 16px", fontSize:13, color:"#E65100" }}>
        🗳️ <strong>Tienes {VOTES_PER_COL} votos por categoría ({VOTES_PER_COL*4} en total).</strong> Úsalos en los puntos más críticos.
      </div>}
                  ...
                  <>
                    ...
                    {lastFilled && (
                      <div style={{ marginTop:20 }}>
                        ...
                        {alreadyCreated
                          ? <div style={{ background:"#E3F2FD", borderRadius:8, padding:10, fontSize:13, color:"#1565C0", textAlign:"center" }}>✅ Ya existe una acción creada — revísala en ✅ Acciones</div>
        ...
        {tab==="resumen" && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                        ...
                        return (
                          <div key={note.id} style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr auto 1fr", gap:8, alignItems:"center", marginBottom:12, background:"#FAFAFA", borderRadius:8, padding:12 }}>
                            ...
                            <div style={{ background:"#E8F5E9", borderRadius:8, padding:"8px 12px", borderLeft:"3px solid #388E3C" }}>
                              ...
                              <div style={{ fontSize:12 }}>{(causeRoot||"").slice(0,80)}{(causeRoot||"").length>80?"...":""}</div>
                            </div>
                            <div style={{ fontSize:20, color:"#1565C0", fontWeight:700 }}>→</div>
            ...
            <div style={{ background:"#fff", borderRadius:12, padding:20, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
              ...
              {participants.length===0 && <p style={{ color:"#aaa", fontSize:13 }}>Sin participantes registrados.</p>}
              {participants.map(p=>(
