...
const COLUMNS = [
  ...
  },
  {
    ...
    color: '#C8E6C9',
    header: '#66BB6A',
    placeholder: '¿Qué se podría haber hecho diferente?',
  },
  {
    id: 'bien',
    title: '¿Qué Hicimos Bien?',
    color: '#BBDEFB',
    header: '#42A5F5',
    placeholder: '¿Qué funcionó correctamente?',
  },
  {
    id: 'lecciones',
    title: '¿Qué Lecciones Aplican a Otros Proyectos?',
    color: '#F8BBD0',
    header: '#EC407A',
    placeholder: 'Lección transferible...',
  },
];
...
export default function App() {
  ...
  const voteNote = async (colId: string, id: string) => {
    ...
    await updateDoc(doc(db, 'notes', colId, 'items', id), {
      votes: increment(1),
    });
  };
  ...
  // Derived
  const allNotes = COLUMNS.flatMap((c) =>
    Object.values(notes[c.id] || {}).map((n: any) => ({ ...n, col: c.id }))
  );
  const filled = allNotes.filter((n: any) => n.text?.trim());
  const topVoted = [...filled]
    .sort((a: any, b: any) => b.votes - a.votes)
    .slice(0, 5);
  const byArea = AREAS.map((a) => ({
    area: a,
    count: filled.filter((n: any) => n.area === a).length,
  })).filter((x) => x.count > 0);
  const byImpact = IMPACT.map((i) => ({
    impact: i,
    count: filled.filter((n: any) => n.impact === i).length,
  }));
  const actList = Object.values(actions).sort(
    (a: any, b: any) => a.createdAt - b.createdAt
  );

  const PROTECTED = ['votar', 'acciones', 'resumen'];
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h2 style={{ margin: '0 0 6px', color: '#1a237e' }}>
            RCA – Lecciones Aprendidas
          </h2>
          <p style={{ color: '#555', marginBottom: 24, fontSize: 14 }}>
            Proyecto BHP · Completa este board{' '}
            <strong>antes de la reunión</strong>
          </p>
          <input
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && userName.trim() && setNameSet(true)
            }
            style={{
              width: '100%',
              padding: '10px 14px',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {(
            ...
            >
              {label}
              {PROTECTED.includes(t) && !adminUnlocked ? ' 🔒' : ''}
            </button>
          ))}
        </div>
      </div>
      ...
      {tab === 'board' && (
        ...
        >
          📋 <strong>Antes de la reunión:</strong> Agrega tus notas en cada
          columna. Indica el área y nivel de impacto. Sé específico — esto hará
          la discusión presencial mucho más rica.
        </div>
      )}
      {tab === 'votar' && adminUnlocked && (
        ...
        >
          🗳️ <strong>Votación (reunión presencial):</strong> Dale 👍 a los
          puntos más críticos. 1 voto por nota por persona.
        </div>
      )}
      {tab === 'acciones' && adminUnlocked && (
        ...
        >
          ✅ <strong>Plan de Acción (al final de la reunión):</strong> Cada
          acción debe tener un responsable y fecha.
        </div>
      )}
      ...
      <div style={{ padding: '16px 24px' }}>
          ...
          >
            ...
            >
              ...
              <p style={{ color: '#666', fontSize: 14, marginBottom: 24 }}>
                Esta sección está reservada para la{' '}
                <strong>reunión presencial</strong>.<br />
        ...
        {tab === 'board' && (
          ...
          >
            {COLUMNS.map((col) => {
              const colNotes = Object.values(notes[col.id] || {}).sort(
                (a: any, b: any) => a.createdAt - b.createdAt
              );
              return (
                <div
                    ...
                    {(colNotes as any[]).map((note) => (
                      ...
                      >
                        ...
                        <textarea
                          ...
                          onBlur={(e) =>
                            updateNote(col.id, note.id, 'text', e.target.value)
                          }
                          style={{
                            width: '100%',
                            border: 'none',
        ...
        {tab === 'votar' && adminUnlocked && (
          ...
          >
            {COLUMNS.map((col) => {
              const colNotes = Object.values(notes[col.id] || {})
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <button
                              onClick={() => voteNote(col.id, note.id)}
                              style={{
                                background: voted[key]
                                  ? '#1565C0'
                                  : 'rgba(255,255,255,0.6)',
                                color: voted[key] ? '#fff' : '#333',
                                border: '1px solid rgba(0,0,0,0.15)',
                                borderRadius: 20,
                                padding: '3px 12px',
                                cursor: voted[key] ? 'default' : 'pointer',
                                fontSize: 12,
                                fontWeight: 700,
                              }}
                            >
        ...
        {tab === 'acciones' && adminUnlocked && (
          <div>
            ...
            >
              ...
              >
                ✅ Plan de Acción — ¿Qué haremos diferente?
              </div>
                      ...
                      >
                        ...
                        <td style={{ padding: '8px 12px' }}>
                          <input
                            defaultValue={a.what}
                            onBlur={(e) =>
                              updateAction(a.id, 'what', e.target.value)
                            }
                            placeholder="¿Qué se va a hacer exactamente?"
                            style={{
                              width: '100%',
                              border: 'none',
                        ...
                        <td style={{ padding: '8px 12px' }}>
                          <input
                            defaultValue={a.who}
                            onBlur={(e) =>
                              updateAction(a.id, 'who', e.target.value)
                            }
                            placeholder="Nombre"
                            style={{
                              width: 120,
                        ...
                        <td style={{ padding: '8px 12px' }}>
                          <input
                            ...
                            defaultValue={a.when}
                            onChange={(e) =>
                              updateAction(a.id, 'when', e.target.value)
                            }
                            style={{
                              border: 'none',
                              borderBottom: '1.5px solid #C8E6C9',
                        ...
                        <td style={{ padding: '8px 12px' }}>
                          <select
                            ...
                            onChange={(e) =>
                              updateAction(a.id, 'priority', e.target.value)
                            }
                            style={{
            ...
            {topVoted.length > 0 && (
              ...
              >
                <h4 style={{ margin: '0 0 10px', color: '#333', fontSize: 14 }}>
                  🔥 Temas más votados — prioriza acciones aquí
                </h4>
        ...
        {tab === 'resumen' && adminUnlocked && (
          ...
          >
            ...
            >
              {[
                ...
                },
                {
                  ...
                  val: [
                    ...new Set(
                      filled.map((n: any) => n.author).filter(Boolean)
                    ),
                  ].length,
                  icon: '👥',
                  color: '#E8F5E9',
                },
                {
                  ...
                  val: actList.filter((a: any) => a.what?.trim()).length,
                  icon: '✅',
                  color: '#FFF8E1',
                },
                {
                  label: 'Votos emitidos',
                  val: filled.reduce((s: number, n: any) => s + n.votes, 0),
                  icon: '🗳️',
                  color: '#FCE4EC',
                },
              ].map((s) => (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 28, fontWeight: 800, color: '#1a237e' }}
                  >
                    {s.val}
                  </div>
            ...
            >
              <h4 style={{ margin: '0 0 14px', color: '#333' }}>
                📊 Notas por categoría
              </h4>
              {COLUMNS.map((col) => {
                const cnt = Object.values(notes[col.id] || {}).filter(
                  (n: any) => n.text?.trim()
                ).length;
                const max = Math.max(
                  ...COLUMNS.map(
                    (c) =>
                      Object.values(notes[c.id] || {}).filter((n: any) =>
                        n.text?.trim()
                      ).length
                  ),
                  1
                ...
                return (
                  <div key={col.id} style={{ marginBottom: 10 }}>
                    ...
                    <div
                      style={{ background: '#eee', borderRadius: 4, height: 8 }}
                    >
                      <div
                        style={{
                          background: col.header,
            ...
            >
              <h4 style={{ margin: '0 0 14px', color: '#333' }}>
                🏗️ Áreas con más incidencias
              </h4>
              {byArea.length === 0 && (
                <p style={{ color: '#aaa', fontSize: 13 }}>
                  Asigna áreas a las notas para ver este gráfico.
                </p>
              )}
              {[...byArea]
              ...
              {byImpact.map(({ impact, count }) => (
                ...
                >
                  ...
                  >
                    <div
                      style={{
                        background: impact.includes('Alto')
                          ? '#e53935'
                          : impact.includes('Medio')
                          ? '#FFA726'
                          : '#66BB6A',
                        height: 10,
                        borderRadius: 4,
                        width: `${(count / Math.max(filled.length, 1)) * 100}%`,
                      }}
                    />
                  </div>
            ...
            >
              <h4 style={{ margin: '0 0 14px', color: '#333' }}>
                🏆 Top 5 puntos más votados
              </h4>
              {topVoted.length === 0 && (
                <p style={{ color: '#aaa', fontSize: 13 }}>Aún no hay votos.</p>
              )}
              {(topVoted as any[]).map((n, i) => {
