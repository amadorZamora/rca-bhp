...
// Simple hash (not for production secrets — use Firebase Auth for production)
...
export default function App() {
  ...
  // ── Admin panel tabs
  const [adminTab,    setAdminTab]    = useState<"projects"|"new"|"edit"|"admins">("projects");
  const [editingProj, setEditingProj] = useState<any>(null);
  const [newProjName, setNewProjName] = useState("");
  const [newProjDesc, setNewProjDesc] = useState("");

  // ── Firestore: admins listener
  useEffect(() => {
  ...
  const handleLogin = async () => {
    ...
    if (adminDoc.exists()) {
      ...
      // Regular user
      if (!loginName.trim()) { setLoginErr("Ingresa tu nombre."); setLoginLoading(false); return; }
      setIsAdmin(false);
  ...
  const createAdmin = async () => {
    setAdminMsg("");
    if (!newAdminName.trim() || !newAdminEmail.trim() || !newAdminPw.trim()) {
      return setAdminMsg("⚠️ Completa todos los campos.");
    }
    ...
    await setDoc(doc(db,"admins",email), {
      ...
      email,
      password: simpleHash(newAdminPw.trim()),
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1565C0,#0D47A1)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:20, padding:40, maxWidth:420, width:"90%", boxShadow:"0 8px 40px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div>
            <label style={{ fontSize:12, color:"#555", fontWeight:600 }}>Contraseña <span style={{ color:"#aaa", fontWeight:400 }}>(solo admins)</span></label>
          ...
          <button onClick={handleLogin} disabled={loginLoading}
            style={{ background:loginLoading?"#90CAF9":"#1565C0", color:"#fff", border:"none", borderRadius:8, padding:"12px", fontSize:15, cursor:"pointer", fontWeight:700, marginTop:4 }}>
            {loginLoading ? "Verificando..." : "Entrar →"}
          </button>
          <p style={{ fontSize:11, color:"#aaa", textAlign:"center", margin:0 }}>
            Los usuarios comunes solo ingresan nombre y correo. La contraseña es solo para administradores.
          </p>
        </div>
      </div>
    </div>
        ...
        {adminTab==="projects" && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            {projects.filter(p=>p.status!=="deleted").length===0 && (
              <div style={{ background:"#fff", borderRadius:12, padding:40, textAlign:"center", color:"#aaa" }}>No hay proyectos aún. Crea el primero →</div>
            )}
          ...
          <div style={{ background:"#fff", borderRadius:12, padding:28, maxWidth:500, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div style={{ background:"#E3F2FD", borderRadius:8, padding:12, fontSize:13, color:"#0D47A1" }}>
                💡 Se creará con solo el Board activo. Activa el resto desde la lista de proyectos.
              </div>
        ...
        {adminTab==="admins" && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, maxWidth:900 }}>
            ...
            <div style={{ background:"#fff", borderRadius:12, padding:24, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
              ...
              {admins.length===0 && <p style={{ color:"#aaa", fontSize:13 }}>No hay admins registrados aún.</p>}
              {admins.map(a=>(
            ...
            <div style={{ background:"#fff", borderRadius:12, padding:24, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                <div style={{ background:"#FFF8E1", borderRadius:8, padding:10, fontSize:12, color:"#E65100" }}>
                  ⚠️ La contraseña se guarda con hash simple. Para mayor seguridad en producción, usa Firebase Authentication.
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ padding:32, maxWidth:700, margin:"0 auto" }}>
        ...
        {projects.filter(p=>p.status!=="deleted").length===0 && (
          <div style={{ background:"#fff", borderRadius:12, padding:32, textAlign:"center", color:"#aaa" }}>
            No hay proyectos disponibles. El administrador debe crear uno.
          </div>
        )}
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {projects.filter(p=>p.status!=="deleted").map(proj=>(
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8", fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      {tab==="board" && <div style={{ background:"#E3F2FD", borderLeft:"4px solid #1565C0", margin:"16px 24px 0", borderRadius:8, padding:"10px 16px", fontSize:13, color:"#0D47A1" }}>
        📋 <strong>Antes de la reunión:</strong> Agrega tus notas. Indica área y nivel de impacto.
      </div>}
              ...
              return (
                <div key={col.id} style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
                  ...
                  <div style={{ padding:12, display:"flex", flexDirection:"column", gap:10, maxHeight:520, overflowY:"auto" }}>
                    ...
                    <button onClick={()=>addNote(col.id)} style={{ background:"rgba(0,0,0,0.05)", border:"2px dashed rgba(0,0,0,0.15)", borderRadius:8, padding:8, cursor:"pointer", fontSize:13, color:"#555" }}>+ Agregar nota</button>
                  </div>
                </div>
              );
            })}
            ...
            <div style={{ background:"#fff", borderRadius:12, padding:24, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
              {!selectedNote ? (
                <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100%", flexDirection:"column", color:"#aaa" }}>
                  <div style={{ fontSize:48 }}>👈</div><p style={{ fontSize:14 }}>Selecciona una nota para aplicar los 5 Porqués</p>
                </div>
              ):(()=>{
